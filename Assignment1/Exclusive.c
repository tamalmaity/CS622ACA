...
void invalid_l3(unsigned long long int block_addr)
{
...
//Eviction in L2 places the block in L3
void invoke_l2_replace(unsigned long long int block_addr)
{
   int i,ind,empty_flag=0; //ind is the index which we want to replace
   int maxm = -1,l2_set=block_addr%1024;
   unsigned long long int evict_block,tag2=block_addr/1024;
   for (i=0;i<8;i++)
   {
  ...
  evict_block=evict_block>>1;
   for(i=0;i<16;i++)
   {
      if(L3[l3_set][i].valid==0)
      { 

         L3[l3_set][i].address=evict_block;
         L3[l3_set][i].lru=0;
         L3[l3_set][i].valid=1;
   ...
   if(!empty_flag)  
   {  evict_block=(evict_block<<11)+l3_set;
      invoke_l3_replace(evict_block);  
   } 

    
}

//L3 hit places block in L2 and invalides block in L3
...
//L3 miss places block in L2 only
void miss_l3(unsigned long long int block_addr)
{

   int i,empty_flag=0,l2_set;
   unsigned long long int tag2=block_addr/1024;
